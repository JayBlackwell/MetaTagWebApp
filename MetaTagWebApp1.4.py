...
def main():
    ...
    with tab1:
        ...
        # Process button with spinner; disable if there are already processed files
        if st.button(
            "Analyze Images", 
            type="primary", 
            disabled=(not uploaded_files or not api_key or len(st.session_state.processed_files) > 0),
            key="analyze_button"
        ):
            if not api_key:
        ...
        if st.session_state.processed_files:
            ...
            num_download_columns = min(max(2, num_processed_files // 4), 6)
    ...
    with tab2:
        ...
        with col1:
            if uploaded_files:
                ...
                num_files = len(uploaded_files)
                num_columns = min(max(2, num_files // 4), 6)
                cols = st.columns(num_columns)
                st.write("Uploaded files:")
                for i, file in enumerate(uploaded_files):
                    with cols[i % num_columns]:
        ...
        if st.button(
            "Remove Metadata", 
            type="primary", 
            disabled=(not uploaded_files or len(st.session_state.processed_files) > 0),
            key="remove_button"
        ):
